...
public class DownloadBaseAction {
	...
	public void download(ByteArrayOutputStream byteArrayOutputStream, HttpServletResponse response, String returnName) throws IOException{
		response.setContentType("application/octet-stream;charset=utf-8");
		//returnName = response.encodeURL(new String(returnName.getBytes(),"iso8859-1"));			//保存的文件名,必须和页面编码一致,否则乱码  针对tomcat6 增加端口的属性 URIEncoding="UTF-8"
		//returnName = new String(returnName.getBytes(),"iso8859-1");
		
		returnName = URLEncoder.encode(returnName,"utf-8");				//中文乱码
		response.addHeader("Content-Disposition",   "attachment;filename=" + returnName);  
