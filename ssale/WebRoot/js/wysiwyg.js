...
var WYSIWYG = {
	...
	"italic":         ['Italic',               '斜体',             	'italics.gif',            'italics_on.gif'],
	...
	_generate: function(n, settings) {
	    ...
	    editor += '<tr><td style="height:0px;vertical-align:top;">';
...
//	    		editor += '<tr><td style="width:6px;"><img src="' + this.config[n].ImagesDir + 'seperator2.gif" alt="" hspace="3"></td>';
		...
		if(get("textMode" + n)) {
			get("textMode" + n).style.display = 'none'; 
		}
	...
	updateStatusBar: function(n) {
		...
		// update the status bar 	
		var statusbar = get("wysiwyg_statusbar_" + n);
		if(statusbar){ 
	...
	writeDropDown: function(n, id) {
		...
		output += '<td onMouseOver="get(\'img_' + dropdown.id + '_' + n + '\').src=\'' + imageOn + '\';" onMouseOut="get(\'img_' + dropdown.id + '_' + n + '\').src=\'' + image + '\';">';
	...
	closeDropDowns: function(n, exid) {
		...
		for(var id in dropdowns) {
			...
			if(dropdown.id != exid) {
				...
				if(get(divId)) get(divId).style.display = 'none';
			}
		}			
	},
	...
	openDropDown: function(n, id) {
		...
		get(divId).style.position = "absolute";
	...
	viewSource: function(n) {
		...
		if(get('HTMLMode' + n)) {
		    get('HTMLMode' + n).style.display = 'none'; 
		}
	    if(get('textMode' + n)) {
		    get('textMode' + n).style.display = 'block';
		}
	...
	viewText: function(n) { 
		...
		if(get('HTMLMode' + n)) {
			get('HTMLMode' + n).style.display = 'block';
		}
